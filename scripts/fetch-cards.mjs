...
// --- Tuning ---
const CHUNK_CONCURRENCY = 2;     // 1=seriell, 2-3 parallel (bei Drossel runtersetzen)
const WRITE_PAUSE_MS = 5;        // Mini-Pause zwischen Dateischreibungen
...
async function main(){
  ...
  // Range-Queue mit limitierter Parallelität
  let idx=0;
  async function worker(){
    while (idx < DEX_CHUNKS.length) {
      ...
      // Zwischenergebnis dieses Chunks auf Platte schreiben
      await writeChunkFiles({start, end}, numToName, bucket);

      // optional: partial index schreiben
